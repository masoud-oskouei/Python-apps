...
class Item(object):
    def __init__(self, master, dsc, unit_price=0):
        self.entry_box = Entry(master, width=3, relief="groove")
        self.v = BooleanVar()
        self.v.set(False)
        self.check_button = Checkbutton(
            master, text=dsc, variable=self.v, borderwidth=4, relief="groove"
        )
        self.unit_price = unit_price
        self.unit_price_label = Label(
            master, text="$" + str(self.unit_price), relief="groove"
        )
...
class Application(Frame):
    ...
    def create_widgets(self):
        self.label1 = Label(
            self,
            text="Please tick what you want and enter the quantity",
            relief="groove",
        )
        self.label1.grid(row=0, column=0, columnspan=2)
        self.label2 = Label(self, text="Quantity:", relief="groove")
        self.label2.grid(row=1, column=0, sticky=W)

        self.label3 = Label(self, text="Food Item:", relief="groove")
        ...
        Label(self, text="Total:", relief="groove").grid(row=8, column=1)
