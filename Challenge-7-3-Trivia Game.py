...
def  getchoice(a_c_list):
    ...
    if a_c_list==[]: return ""
    for a_i in a_c_list:
        if not  isinstance(a_i[0],str) or not isinstance(a_i[1],str) \
        or not isinstance(a_i,tuple) or len(a_i)!=2:
    ...
    while a_i<len(a_c_list):
        ...
        while a_j<len(a_c_list):
            if a_c_list[a_i][0]==a_c_list[a_j][0] or a_c_list[a_i][1]==a_c_list[a_j][1]:
    ...
    while a_breakloop ==None: 
        for a_i in a_c_list:  print(a_i[0],"-",a_i[1])
        a_gc=input("please enter your choice  ")
        for a_i in a_c_list:
            if a_i[0]==a_gc:
                a_breakloop=1
                a_chosen=a_i
        if a_breakloop!=1: print("Please enter a correct number! ")                               
...
def fill():
    ...
    import shelve
    b_s=shelve.open("game_database.dat")
    
    for b_i in range(102,122):b_s[str(b_i)]=["",[("",""),("",""),("",""),("","")],"","",""]
    b_s["101"]=["This game is about Movies. "]
    b_s["102"]=["1. Who was the director of 'The birds'?",\
               [("a","Hitchcock"),("b","Soderberg"),("c","Spilberg"), ("d","Eastwood")],\
              "a","It was a horror film by Horror King Mr. Hitchcock","10"]
    b_s["103"]=["2. Who played the 'Raging Bull?",\
               [("a","Cruze"),("b","Deniro"),("c","Schwartzeneger"),("d","Stalone")],\
              "b","The film was in black and white","15"]
    b_s["122"]=["Thank you for playing this game"]
    
    for b_i in range(202,222):b_s[str(b_i)]=["",[("",""),("",""),("",""),("","")],"","",""]
    b_s["201"]=["This game is about Computers. "]
    b_s["202"]=["1. Which was introduced after Windows95?",\
              [("a","Win98") ,("b","Win7") ,("c","Win_XP") ,("d","WinNT")],\
              "d","It was intended for servers","11"]
    b_s["203"]=["2. How many cores a PentiumII consists of?",\
               [("a","1") ,("b","2") ,("c","4") ,("d","6")],\
              "a","It was introduced in 90's","12"]
...
def play_game(c_game):
    c_user_score=0
    import shelve
    c_s=shelve.open("game_database.dat")
    print(c_s[c_game+"01"])
...
def high_score(e_score):
    ...
    e_file=open("score_file.txt","r")
    e_len=0
    for e_l in e_file: e_len+=1
    e_file.close()
    print("e_len=",e_len)
    e_file=open("score_file.txt","r+")
    for e_l in range(1,e_len): print(e_file.readline())
    e_char=""
    e_prev_high_name=""
    e_prev_high_score=""
    ...
    while e_char!=",":
        ...
        if e_char!=",": e_prev_high_name+=e_char
    while e_char!="\n":  
        e_char=e_file.read(1)
        if e_char!="\n": e_prev_high_score+=e_char
    print("e_prev_high_name=",e_prev_high_name,"e_prev_high_score=",e_prev_high_score)   
...
def main():
    ...
    import shelve
    import pickle
    m_s=shelve.open("game_database.dat")
    print(m_s["103"][0])
    m_menu=[("1","Movie Game"),("2","Computer Game"),\
            ("3","Edit Database"),("4","Reset high scores"),("5","Show high scores"),("6","Exit")]
    m_choice="1"
    while m_choice[0] in ["1","2","3","4","5"]:
        ...
        if m_choice[0]=="5":
            ...
            m_len=0
            for m_l in m_file: m_len+=1
            m_file.close()
            m_file=open("score_file.txt","r")
            for m_l in range(1,m_len-1): m_file.readline()
