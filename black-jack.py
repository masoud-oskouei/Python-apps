...
class Bj_player(Bj_hand):
    ...
    def is_hitting(self):
        response = games.ask_yn(
            "Dealer says to " + self.name + ": \ndo you want a hit?"
        )
        return response == "y"
    ...
    def lose(self):
        ...
        print(self.name, " lost ", self.bet, " dollars ! ")
        self.money -= self.bet
        print("Now ", self.name, " has ", self.money, " dollars ! ")
        if self.money == 0:
            print(self.name, " has no money now ! ")

    def win(self):
        # print(self.name," wins!")
        print(self.name, " Won ", self.bet, " dollars ! ")
        self.money += self.bet
        print("Now ", self.name, " has ", self.money, " dollars ! ")
...
class Bj_game(object):
    ...
    def ask_bet(self, player):
        ...
        player.bet = int(
            games.ask_num(
                f"Dealer to {player.name} says: "
                + "\nHow much dollars are"
                + " you going to bet?(from 0 upto your money)",
                0,
                player.money,
            )
        )
...
def main():
    screen_clear()
    print("\t\t\t-=Welcome to This very Good Casino!=-\n")
    persons = []
    number = int(
        games.ask_num(
            "How many players are going to play? Please press enter after typing the number... : ",
            low=1,
            high=7,
        )
    )
    for i in range(number):
        ...
        money = int(games.ask_num(f"Enter the money of player-{i+1}: ", 0, 1000000))
